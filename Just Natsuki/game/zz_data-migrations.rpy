...
python early in jn_data_migrations:
    ...
    def checkCurrentVersionIsLatest(notify_if_current=False):
        ...
        try:
            ...
            if response.status_code != 200:
                jn_utils.log("No se pudieron buscar actualizaciones: la respuesta de los lanzamientos de GitHub fue: {0}".format(response.status_code))
                return False
            ...
            if not current_version_latest:
                renpy.notify("¡Ya está disponible una actualización para Just Natsuki! Ir a: https://github.com/Just-Natsuki-Team/NatsukiModDev/releases (Version Inglesa)")
            
            elif notify_if_current:
                renpy.notify("¡Just Natsuki ya está actualizado!")
        
        except Exception as exception:
            jn_utils.log("No se pudieron buscar actualizaciones: {0}".format(exception))
            return False
    ...
    def runInitMigrations():
        ...
        """
        jn_utils.log("runInitMigrations INICIADA")
    ...
    def runRuntimeMigrations():
        ...
        """
        jn_utils.log("runRuntimeMigrations INICIADA")
        for _callable in LATE_UPDATES:
            _callable()


init 10 python:
    jn_utils.log("Comprobación previa a la migración de la versión persistente actual: {0}".format(store.persistent._jn_version))
...
init python in jn_data_migrations:
    ...
    def to_1_0_0():
        jn_utils.log("Migración a 1.0.0 INICIADA")
        ...
        store.persistent._topic_database["talk_mod_contributions"]["conditional"] = "not jn_activity.ACTIVITY_SYSTEM_ENABLED or jn_activity.ACTIVITY_MANAGER.hasPlayerHECHAActivity(jn_activity.JNActivities.coding)"
        store.get_topic("talk_mod_contributions").conditional = "not jn_activity.ACTIVITY_SYSTEM_ENABLED or jn_activity.ACTIVITY_MANAGER.hasPlayerHECHAActivity(jn_activity.JNActivities.coding)"
        ...
        jn_utils.saveGame()
        jn_utils.log("Migración a 1.0.0 HECHA")
        return

    @migration(["1.0.0"], "1.0.1", runtime=MigrationRuntimes.INIT)
    def to_1_0_1():
        jn_utils.log("Migración a 1.0.1 INICIADA")
        ...
        jn_utils.saveGame()
        jn_utils.log("Migración a 1.0.1")
        return

    @migration(["1.0.1"], "1.0.2", runtime=MigrationRuntimes.INIT)
    def to_1_0_2():
        jn_utils.log("Migración a 1.0.2 INICIADA")
        store.persistent._jn_version = "1.0.2"
        jn_utils.log("Migración a 1.0.2 HECHA")
        return

    @migration(["1.0.2"], "1.0.3", runtime=MigrationRuntimes.INIT)
    def to_1_0_3():
        jn_utils.log("Migración a 1.0.3 INICIADA")
        ...
        jn_utils.saveGame()
        jn_utils.log("Migración a 1.0.3 HECHA")
        return

    @migration(["1.0.3", "1.0.4"], "1.1.0", runtime=MigrationRuntimes.INIT)
    def to_1_1_0():
        jn_utils.log("Migración a 1.1.0 INICIADA")
        ...
        jn_utils.saveGame()
        jn_utils.log("Migración a 1.1.0 HECHA")
        return

    @migration(["1.1.0"], "1.1.1", runtime=MigrationRuntimes.INIT)
    def to_1_1_1():
        jn_utils.log("Migración a 1.1.1 INICIADA")
        store.persistent._jn_version = "1.1.1"
        jn_utils.log("Migración a 1.1.1 HECHA")
        return

    @migration(["1.1.1", "1.1.2"], "1.2.0", runtime=MigrationRuntimes.INIT)
    def to_1_2_0():
        jn_utils.log("Migración a 1.2.0 INICIADA")
    ...
    def to_1_2_4():
        ...
        jn_utils.saveGame()
        jn_utils.log("Migración a 1.2.4 HECHA")
        return

    @migration(["1.2.4"], "1.3.0", runtime=MigrationRuntimes.INIT)
    def to_1_3_0():
        jn_utils.log("Migración a 1.3.0 INICIADA")
    ...
    def to_1_3_5():
        ...
        jn_utils.log("Migración a 1.3.5 HECHA")
        return
