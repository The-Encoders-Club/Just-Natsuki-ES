...
label music_menu:
    ...
    if not success:
        ...
        menu:
            n "¿Quieres que vuelva a subir el volumen de la música para que puedas escoger algo?"
            "Sí":
    ...
    if _return.option_type == jn_custom_music.JNMusicOptionTypes.no_music:
        ...
        $ chosen_question_quip = renpy.substitute(random.choice([
            "¿Aja? {w=0.2}¿Quieres que yo escoja?",
            "¿Eh? {w=0.2}¿Quieres que elija algo?",
            "¿Mmm? {w=0.2}¿Quieres que la elija?",
        ...
        $ chosen_answer_quip = renpy.substitute(random.choice([
            ...
            "¡Puedo hacerlo!",
            "Jeh. {w=0.2}Déjamelo a mí, {w=0.2}[player]!",
        ...
        $ music_quip = renpy.substitute(random.choice([
            ...
            "Solo lo habitual, {w=0.2}¿eh? {w=0.2}¡Seguro!",
