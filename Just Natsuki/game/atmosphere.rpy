...
init python in jn_atmosphere:
    ...
    WEATHER_RAIN = JNWeather(
        ...
        notify_text=[
            _("Ugh... está lloviendo otra vez. [angry_emote]"),
            _("Qué asco. Lluvia. [angry_emote]"),
            _("Hombre... ¿por qué tiene que llover tanto?"),
            _("¿Eh? ¿Está lloviendo? Qué asco... [angry_emote]"),
        ],
    ...
    WEATHER_THUNDER = JNWeather(
        ...
        notify_text=[
            _("Esas nubes se ven muy oscuras ¿no? :<"),
            _("Es una noche oscura y tormentosa..."),
            _("Ugh... Odio las tormentas... [angry_emote]"),
            _("Oye... ¿también hay tormenta allí? :/"),
        ],
        dim_image="dim heavy",
    ...
    WEATHER_SNOW = JNWeather(
        ...
        night_sky_image="sky night overcast",
        notify_text=[
            _("¡[player]! ¡[player]! ¡Está nevando! [happy_emote]"),
            _("¡Está nevando! ¡Está nevando! [happy_emote]"),
            _("¡[player]! ¡¿También está nevando por tu lado?! [happy_emote]"),
        ],
        dim_image="dim light",
    ...
    def updateSky(with_transition=True):
        ...
        if store.persistent._jn_weather_setting == int(jn_preferences.weather.JNWeatherSettings.real_time):
            ...
            if not api_weather_result:
                jn_utils.log("No se puede recuperar el clima de la API; el valor predeterminado es Soleado.")
                renpy.notify("No se pudo actualizar el clima; consulte el registro para obtener más información.")
    ...
    def getWeatherFromApi():
        ...
        try:
            ...
            jn_utils.log("No se puede obtener el clima de OpenWeatherMap debido a que ocurrió una excepción; {0}".format(exception.message))
            return None
        
        if weather_response.status_code != 200:
            
            jn_utils.log("No se puede obtener el clima de OpenWeatherMap; la respuesta de la API fue: {0}".format(weather_response.status_code))
            return None
    ...
    def getLatitudeLongitudeByIpAddress():
        ...
        try:
            ...
            jn_utils.log("No se pudo recuperar la latitud y longitud del usuario a través de la dirección IP: {0}".format(exception))
            return None
