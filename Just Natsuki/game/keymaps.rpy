init -50 python:
    def jn_register_label_keymap(keymap_name, keymap_label, *keys):
        ...
        if not renpy.has_label(keymap_label):
            jn_utils.log("[ERROR]: Se intentó registrar el mapa de teclas de etiqueta para una etiqueta que no existe", jn_utils.SEVERITY_WARN)
    ...
    def jn_register_keymap(keymap_name, keymap_action, *keys):
        ...
        if keymap_name in config.keymap:
            jn_utils.log("ERROR: Se intentó registrar un nuevo mapa de teclas con un nombre existente. Posponiendo.", jn_utils.SEVERITY_WARN);
            return
        
        
        if not callable(keymap_action):
            jn_utils.log("ERROR: La acción del mapa de teclas proporcionada no se puede llamar. Posponiendo.", jn_utils.SEVERITY_WARN)
            return
