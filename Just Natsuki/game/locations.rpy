...
init -20 python:
    ...
    class JNLocation(object):
        ...
        def __init__(
            ...
            if not renpy.loadable(day_image_fp):
                raise Exception("[ERROR]: La Imagen de dia ('{0}') no se puede cargar".format(day_image_fp))
            
            
            if not renpy.loadable(night_image_fp):
                raise Exception("[ERROR]: La imagen de noche ('{0}') no se puede cargar.".format(night_image_fp))
    ...
    class Location(object):
        ...
        def __init__(
            ...
            if not renpy.loadable(day_image_fp):
                raise Exception("[ERROR]: La imagen del dia ('{0}') no se puede cargar.".format(day_image_fp))
    ...
    class JNRoom(object):
        ...
        def draw(self, dissolve_all=False, full_redraw=False):
            ...
            if room is not None:
                ...
                jn_utils.log("No se puede mostrar la habitación: no se encontró ninguna imagen de la habitación.")
